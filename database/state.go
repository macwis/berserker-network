...
// mempool simplified - to consider for prod:
//  - max size
...
func (s *State) AddBlock(b Block) (Hash, error) {
	...
	// All TXs are valid and no error writing to disk. Updating the state.
...
func applyTx(tx Tx, s *State) error {
	...
	if tx.Value > s.Balances[tx.From] {
		return fmt.Errorf("wrong TX. Sender '%s' balance is %d BRS. Tx cost is %d BRS", tx.From, s.Balances[tx.From], tx.Value)
	}
